...
# function frequency
hedge = 60 # delta hedging frequency (minutes)
status = 90 # account update notification frequency (minutes)

# order sizing 
leverage = 1 # (* underlying price * 100) - the lower the number, the larger the position size

# liquidate positions before close (False) or take assignment (True)
assign = False

# schedule closing functions (military time)
hour = 14 # stop hedging at (hour)
...
print("")
display(HTML(f'<b>{trade_type} Variance Premium</b>'))
print("")
...
def OpenOrders():
    display(HTML('<b>SELL STRADDLE</b>'))
    ...
    call_margin = (call_ask * 100) + (leverage * (100 * last_price)) 
    put_margin = (put_ask * 100) + (leverage * (100 * last_price)) 
    ...
    if call_strike == put_strike and call_filled == False and put_filled == False:
        ...
        call_order_req = requests.post(option_order_url,
            data={'class': 'option', 'symbol': ticker, 'option_symbol': call, 'side': 'sell_to_open', 'quantity': order_size, 'type': 'limit', 'duration': 'day', 'price': call_theo, 'tag': 'my-tag-example-1'},
            headers=headers
        )
        ...
        put_order_req = requests.post(option_order_url,
            data={'class': 'option', 'symbol': ticker, 'option_symbol': put, 'side': 'sell_to_open', 'quantity': order_size, 'type': 'limit', 'duration': 'day', 'price': put_theo, 'tag': 'my-tag-example-1'},
            headers=headers
        )
    ...
    if positions['positions'] != 'null':
        ...
        if C is not None:
            close_call_rep = requests.post(option_order_url,
                data={'class': 'option', 'symbol': ticker, 'option_symbol': C, 'side': 'buy_to_close', 'quantity': order_size, 'type': 'limit', 'duration': 'day', 'price': call_bid, 'tag': 'my-tag-example-1'},
                headers=headers
            )
        ...
        if P is not None:
            close_put_rep = requests.post(option_order_url,
                data={'class': 'option', 'symbol': ticker, 'option_symbol': P, 'side': 'buy_to_close', 'quantity': order_size, 'type': 'limit', 'duration': 'day', 'price': put_bid, 'tag': 'my-tag-example-1'},
                headers=headers
            )
...
def Liquidate():
    ...
    if positions['positions'] != 'null':
        ...
        if C is not None:
            call_url = '{}accounts/{}/orders/{}'.format(api_base_url, account_id, closecall_id)
            modify_call_rep = requests.put(call_url, data={'type': 'market', 'duration': ''}, headers=headers)
            modify_call = modify_call_rep.json()
            print(f"API Response Status: {modify_call_rep.status_code}")
            print(modify_call)
            print(json.dumps(modify_call, indent=4, sort_keys=True))
            print("")
        if P is not None:
            put_url = '{}accounts/{}/orders/{}'.format(api_base_url, account_id, closeput_id)
            modify_put_rep = requests.put(put_url, data={'type': 'market', 'duration': ''}, headers=headers)
            modify_put = modify_put_rep.json()
            print(f"API Response Status: {modify_put_rep.status_code}")
            print(modify_put)
            print(json.dumps(modify_put, indent=4, sort_keys=True))
            print("")
...
OpenOrders()
# schedule.every().day.at("10:00").do(OpenOrders)
schedule.every(status).minutes.do(UserAccount)
schedule.every(hedge).minutes.do(DeltaHedge) 
